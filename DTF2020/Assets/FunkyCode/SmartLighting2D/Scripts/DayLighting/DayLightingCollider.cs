...
public class DayLightingCollider {
    ...
    static public void Shadow(Vector2D offset, float z) {
			...
			if (manager.drawSunPenumbra) {
				...
				for(int i = 0; i < shadow.polygonsPairs.Count; i++) {
					...
					for(int x = 0; x < polygonPairs.Count; x++) {
						...
						zA.x += System.Math.Cos(angleA) * manager.sunPenumbra;
						zA.y += System.Math.Sin(angleA) * manager.sunPenumbra;

						zB.x += System.Math.Cos(angleB) * manager.sunPenumbra;
						zB.y += System.Math.Sin(angleB) * manager.sunPenumbra;

						zC.x += System.Math.Cos(angleC) * manager.sunPenumbra;
						zC.y += System.Math.Sin(angleC) * manager.sunPenumbra;
    ...
    static public void Mask(Vector2D offset, float z) {
        manager = LightingManager2D.Get();
		...
		if (manager.lightingSpriteAtlas && SpriteAtlasManager.GetAtlasPage() != null) {
			WithAtlas.Mask(offset, z);
		} else {
			WithoutAtlas.Mask(offset, z);
		}       
	}
