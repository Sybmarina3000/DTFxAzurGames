...
public class LightingSourceTexture {
	...
	public static void Draw(LightingBuffer2D buffer) {
		...
		if (buffer.lightSource.rotationEnabled) {
			...
			Lighting2D.Max2D.DrawImage2DBatched(Vector2.zero, size, buffer.lightSource.transform.rotation.eulerAngles.z, z, buffer.lightSource.spriteFlipX, buffer.lightSource.spriteFlipY);
			...
			Lighting2D.Max2D.DrawImage2D(material, Vector2.zero, size, 0, z, buffer.lightSource.spriteFlipX, buffer.lightSource.spriteFlipY);
		}
	}
}