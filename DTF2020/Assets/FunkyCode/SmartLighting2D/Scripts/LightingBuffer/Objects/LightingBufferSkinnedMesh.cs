...
public class LightingBufferSkinnedMesh {
    static Vector2D zero = Vector2D.Zero();
   	static Vector2D vA = Vector2D.Zero(), pA = Vector2D.Zero(), vB = Vector2D.Zero(), pB = Vector2D.Zero();
	static Vector2D offset = Vector2D.Zero();
	static Rect uvRect = new Rect();
	static Pair2D pair = Pair2D.Zero();
	...
	public static void Shadow(LightingBuffer2D buffer, LightingCollider2D id, float lightSizeSquared, float z) {
		if (id.shape.colliderType != LightingCollider2D.ColliderType.SkinnedMesh) {
			return;
		}
		...
		List<Polygon2D> polygons = id.shape.GetPolygons_World_ColliderType(id.transform);
	...
	public class WithoutAtlas {
		...
		public static void Mask(LightingBuffer2D buffer, LightingCollider2D id, Material material, LayerSetting layerSetting, Vector2D offset, float z) {
			if (id.shape.maskType != LightingCollider2D.MaskType.SkinnedMesh) {	
				return;
			}
