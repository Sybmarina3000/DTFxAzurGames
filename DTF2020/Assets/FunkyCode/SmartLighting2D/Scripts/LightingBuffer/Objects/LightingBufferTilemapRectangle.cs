...
public class LightingBufferTilemapRectangle {
	...
	static public Vector2Int newPositionInt = new Vector2Int();
	static VirtualSpriteRenderer spriteRenderer = new VirtualSpriteRenderer();

	static Vector2 scale = Vector2.zero;
	static Vector2 posScale = Vector2.zero;
	static Vector2D scale2D = Vector2D.Zero();

	public static int sizeInt;

	static Sprite reqSprite;
	static PartiallyBatchedTilemap batched;
	...
	public class WithAtlas {
		static public void MaskSprite(LightingBuffer2D buffer, LightingTilemapCollider2D id, Vector2D offset, float z) {
	...
	public class WithoutAtlas {
		static public void MaskSprite(LightingBuffer2D buffer, LightingTilemapCollider2D id, Material materialA, Material materialB, Vector2D offset, float z) {
			if (id.maskType != LightingTilemapCollider2D.MaskType.Sprite) {
				return;
			}
