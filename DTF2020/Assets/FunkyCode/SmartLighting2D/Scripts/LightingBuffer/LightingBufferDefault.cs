...
public class LightingBufferDefault {
	...
	public static void Draw(LightingBuffer2D buffer, LayerSetting layer) {
		...
		// Drawing
		if (manager.lightingSpriteAtlas && SpriteAtlasManager.GetAtlasPage() != null) {
		...
		public static void Draw(LightingBuffer2D buffer, LayerSetting layer) {
			...
			if (drawShadows) {
				...
				for(int id = 0; id < colliderList.Count; id++) {
					...
					// Collider Shadow
					LightingBufferCollider.Shadow(buffer, colliderList[id], lightSizeSquared, z, offset);
			...
			if (drawMask) {
				if (colliderList.Count > 0) {
					for(int id = 0; id < colliderList.Count; id++) {
						...
						// Collider Shape Mask
						LightingBufferCollider.Mask(buffer, colliderList[id], layer, offset, z);
						...
						LightingBufferSprite.WithAtlas.Mask(buffer, colliderList[id], offset, z);
					}
				}
					...
					for(int id = 0; id < tilemapList.Count; id++) {
						...
						// Tilemap Sprite Mask
						LightingBufferTilemapRectangle.WithAtlas.MaskSprite(buffer, tilemapList[id], offset, z);
					}
				#endif
			}
			...
			if (buffer.partiallyBatchedList_Collider.Count > 0) {
				...
				for(int i = 0; i < buffer.partiallyBatchedList_Collider.Count; i++) {
					...
					LightingBufferSprite.WithoutAtlas.Mask(buffer, batch.collider, material, offset, z);
				}
		...
		static public void Mask(LightingBuffer2D buffer, LayerSetting layer) {
			...
			if (colliderList.Count > 0) {
				for(int id = 0; id < colliderList.Count; id++) {
					...
					LightingBufferSprite.WithoutAtlas.Mask(buffer, colliderList[id], material, offset, z);

					LightingBufferMesh.WithoutAtlas.Mask(buffer, colliderList[id], material, layer, offset, z);

					LightingBufferSkinnedMesh.WithoutAtlas.Mask(buffer, colliderList[id], material, layer, offset, z);
				}
			}
			...
			// Tilemap Sprite Mask
				...
				for(int id = 0; id < tilemapList.Count; id++) {
					...
					LightingBufferTilemapRectangle.WithoutAtlas.MaskSprite(buffer, tilemapList[id], materialWhite, materialBlack, offset, z);
				}
			#endif
		}
	}   
