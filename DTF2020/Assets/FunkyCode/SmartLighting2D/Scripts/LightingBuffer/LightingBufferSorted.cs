                ...
                switch (depth.type) {
                    case ColliderDepth.Type.Collider:
                        if ((int)depth.collider.lightingCollisionLayer == layerID && drawShadows) {	
                            LightingBufferCollider.Shadow(buffer, depth.collider, lightSizeSquared, z, offset);
                        }

                        if ((int)depth.collider.lightingMaskLayer == layerID && drawMask) {
                            LightingBufferSprite.WithAtlas.Mask(buffer, depth.collider, offset, z);

                            LightingBufferCollider.Mask(buffer, depth.collider, layer, offset, z);  
                            ...
                            if (buffer.partiallyBatchedList_Collider.Count > 0) {
                                ...
                                for(int s = 0; s < buffer.partiallyBatchedList_Collider.Count; s++) {
                                    ...
                                    LightingBufferSprite.WithoutAtlas.Mask(buffer, batch_collider.collider, material, offset, z);
            ...
            for(int i = 0; i < list.count; i ++) {
                ...
                switch (depth.type) {
                    ...
                    if ((int)depth.collider.lightingMaskLayer == layerID && drawMask) {
                            ...
                            LightingBufferSprite.WithoutAtlas.Mask(buffer, depth.collider, material, offset, z);
