...
public class LightingManager2DEditor : Editor {
	...
	override public void OnInspectorGUI() {
		...
		if (foldout_settings) {
			...
			script.drawPenumbra = EditorGUILayout.Toggle("Draw Light Penumbra", script.drawPenumbra);

			script.drawSunPenumbra = EditorGUILayout.Toggle("Draw Sun Penumbra", script.drawSunPenumbra);

			script.drawHighQualityShadows = EditorGUILayout.Toggle("High Quality Shadows", script.drawHighQualityShadows);
		...
		if (foldout_daylighting) {
			...
			float sunPenumbra = EditorGUILayout.FloatField("Sun Penumbra", script.sunPenumbra);
			if (sunPenumbra < 0) {
				sunPenumbra = 0;
			}
			if (sunPenumbra != script.sunPenumbra) {
				script.sunPenumbra = sunPenumbra;
			}
		...
		if (foldout_lightingAtlas) {
			...
			script.SpriteAtlasSize = (SpriteAtlasSize)EditorGUILayout.EnumPopup("Atlas Size", script.SpriteAtlasSize);
		...
		if (GUILayout.Button(buttonName)) {
			if (script.mainBuffer != null && script.fboManager != null && script.meshRendererMode != null) {
				...
				if (script.spriteAtlas != null) {
					DestroyImmediate(script.spriteAtlas.gameObject);
				}
