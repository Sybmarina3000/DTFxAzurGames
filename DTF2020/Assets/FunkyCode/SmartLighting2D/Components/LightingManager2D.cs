...
public class LightingManager2D : MonoBehaviour {
	...
	public void Render_MeshRenderMode() {
		...
		if (meshRendererMode.meshRenderer.sharedMaterial != mainBuffer.GetMaterial()) {
			meshRendererMode.meshRenderer.sharedMaterial = mainBuffer.GetMaterial();
		}
		
		
		if (meshRendererMode.meshRenderer.sharedMaterial == null) {
            LightingManager2D manager = LightingManager2D.Get();
			Debug.Log(manager.mainBuffer.GetMaterial());
            meshRendererMode.meshRenderer.sharedMaterial = manager.mainBuffer.GetMaterial();
        }
	}
	...
	void Render_PostRenderMode() {
		...
		Lighting2D.Max2D.DrawImage(mainBuffer.GetMaterial(), Render_Position(), Render_Size(), camera.transform.eulerAngles.z, Render_Position().z);
	}
	...
	public void Render_PreRenderMode() {
		...
		Graphics.DrawMesh(GetRenderMesh(), Matrix4x4.TRS(Render_Position(), rotation, Render_Size()), mainBuffer.GetMaterial(), 0);
	}
